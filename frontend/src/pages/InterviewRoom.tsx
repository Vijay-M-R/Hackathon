          ...
          <Card className="border-2 border-primary/20">
            <CardHeader className="bg-primary/5 pb-8">
              <div className="flex justify-between items-start">
                <div>
                  <CardTitle className="text-3xl font-bold">{interview.title} - Analysis</CardTitle>
                  <CardDescription className="text-lg">Final Report & Feedback</CardDescription>
                </div>
                <div className="text-right">
                  <div className="text-sm font-medium text-muted-foreground mb-1">Overall Readiness Impact</div>
                  <div className="text-5xl font-black text-primary">{Math.round(interview.overallScore)}%</div>
                </div>
              </div>
            </CardHeader>
            <CardContent className="pt-8 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <h3 className="font-semibold text-xl flex items-center gap-2">
                    <CheckCircle2 className="text-green-500 h-6 w-6" /> Key Strengths
                  </h3>
                  <div className="p-4 rounded-lg bg-green-500/5 border border-green-500/10 text-sm leading-relaxed">
                    {interview.feedback || "Your communication was clear and you demonstrated a solid grasp of fundamental concepts."}
                  </div>
                </div>
                <div className="space-y-4">
                  <h3 className="font-semibold text-xl flex items-center gap-2">
                    <AlertCircle className="text-amber-500 h-6 w-6" /> Improvement Areas
                  </h3>
                  <div className="p-4 rounded-lg bg-amber-500/5 border border-amber-500/10 text-sm leading-relaxed">
                    Try to provide more structured examples (STAR method) and dive deeper into the technical trade-offs of your decisions.
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <h3 className="font-semibold text-xl flex items-center gap-2">
                  <BarChart3 className="text-primary h-6 w-6" /> Detailed Breakdown
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { label: "Technical", val: interview.analysis?.technical || 75 },
                    { label: "Communication", val: interview.analysis?.communication || 82 },
                    { label: "Confidence", val: interview.analysis?.confidence || 70 },
                    { label: "Problem Solving", val: interview.analysis?.problemSolving || 65 }
                  ].map(stat => (
                  ...
                  {messages.map((m, idx) => (
                    <div key={idx} className={`flex flex-col ${m.senderRole === 'STUDENT' ? 'items-end' : 'items-start'}`}>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{m.senderName}</span>
                      </div>
                      <div className={`p-3 rounded-2xl max-w-[80%] text-sm ${
                        m.senderRole === 'STUDENT' ? 'bg-primary text-primary-foreground rounded-tr-none' : 'bg-muted rounded-tl-none'
                      }`}>
                        {m.text}
                      </div>
                    </div>
      ...
      <div className="h-[calc(100vh-140px)] flex flex-col gap-4 max-w-6xl mx-auto">
        ...
        <main className="flex-1 overflow-hidden flex gap-4">
          ...
          <div className="w-72 hidden lg:flex flex-col gap-4">
            ...
            <Card className="flex-1 bg-gradient-to-b from-primary/5 to-transparent">
              <CardHeader className="p-4">
                <CardTitle className="text-sm">AI Insights</CardTitle>
                <CardDescription className="text-[10px]">Real-time sentiment active</CardDescription>
              </CardHeader>
              <CardContent className="p-4 pt-0 space-y-4">
                <div className="space-y-1">
                  ...
                  <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                    <div 
                      className={`h-full transition-all duration-500 ${sentiment.score > 60 ? 'bg-green-500' : sentiment.score < 40 ? 'bg-red-500' : 'bg-amber-500'}`} 
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-[10px] font-bold uppercase tracking-wider">
                    <span>Clarity</span>
                    <span>{Math.round(sentiment.clarity)}%</span>
                  </div>
                  <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                    <div className="h-full bg-primary transition-all duration-500" style={{ width: `${sentiment.clarity}%` }} />
                  </div>
                </div>
                <div className="p-3 rounded-lg bg-background/50 border text-[11px] leading-relaxed text-muted-foreground animate-pulse">
                  {sentiment.score > 70 
